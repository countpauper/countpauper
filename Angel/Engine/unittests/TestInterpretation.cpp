...
TEST(Interpretation, Set)
{
    Logic::Knowledge k; 
    AngelInterpreter interpreter;
    Parser::Source source("s:{ginny, gizmo}");
    interpreter.Interpret(source, k);
...
TEST(Interpretation, BinaryOperation)
{
    Logic::Knowledge k; 
    AngelInterpreter interpreter;
    Parser::Source source("hascat(*C):$C.cat");
    interpreter.Interpret(source, k);
...
TEST(Interpretation, Factorial)
{
    Logic::Knowledge k; 
    AngelInterpreter interpreter;
    Parser::Source source("factorial(≤1)←1\nfactorial($n) ← $n⋅factorial($n-1)");
    interpreter.Interpret(source, k);
...
TEST(Interpretation, Sort)
{
    Logic::Knowledge k; 
    AngelInterpreter interpreter;
    Parser::Source source("sort($H,∀T)←sort(∀T:<$H) + [$H] + sort(∀T:≥$H)\nsort←[]");
    interpreter.Interpret(source, k);
