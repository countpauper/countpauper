...
private:
    friend struct std::hash<Parser::Literal>;
    std::string match;
};
...
private:
    friend struct std::hash<Parser::Regex>;
    std::wstring wmatch;
...
private:
    friend struct std::hash<Parser::Epsilon>;
    Symbol symbol;
};
...
{
template <>
struct hash<Parser::Literal>
{
    size_t operator()(const Parser::Literal& l) noexcept ;
};

template <>
struct hash<Parser::Regex>
{
    size_t operator()(const Parser::Regex& l) noexcept ;
};

template <>
struct hash<Parser::Epsilon>
{
    size_t operator()(const Parser::Epsilon& e) noexcept ;
};

string to_string(const Parser::Term& term);
}