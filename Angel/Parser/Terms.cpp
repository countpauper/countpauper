...
size_t hash<Parser::Literal>::operator()(const Parser::Literal& l) noexcept 
{
    std::hash<std::string> hasher;
    return hasher(l.match);
}

size_t hash<Parser::Regex>::operator()(const Parser::Regex& r) noexcept 
{
    std::hash<decltype(r.wmatch)> hasher;
    return hasher(r.wmatch);
}

size_t hash<Parser::Epsilon>::operator()(const Parser::Epsilon& e) noexcept 
{
    std::hash<std::string> hasher;
