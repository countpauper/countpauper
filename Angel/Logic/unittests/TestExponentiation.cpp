...
TEST(Exponentiation, Construction)
{
    ...
    EXPECT_EQ(GenericOperation(L'↑', Tuple{Integer(-2), Integer(-1)}), (Exponentiation({Integer(-2), Integer(-1)})));   
}

TEST(Exponentiation, Exponentiations)
{
    Knowledge k;
    EXPECT_EQ(k.Infer(Exponentiation({Real(2)})), Real(std::exp(2)));
    EXPECT_EQ(k.Infer(Exponentiation({Integer(3)})), Integer(8));
...
TEST(Exponentiation, Solve)
{
    ...
    // Y = X ↑ 2 => X = Y ↑ (1/2) TODO or -sqrt(Y)
    EXPECT_EQ(Exponentiation({Variable("X"), Integer(2)}).Solve(Variable("X"), Variable("Y")), 
        Exponentiation({Variable("Y"), Division({Integer(1), Integer(2)})}));
}
...
TEST(Exponentiation, to_string)
{
    ...
    EXPECT_EQ(to_string(Exponentiation({Integer(-2), Integer(3), Real(0.5)})), "√(-2↑-3)");
