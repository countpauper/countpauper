...
public:
    MockActor() :
        ...
        counts(stats)
    {
        ON_CALL(*this, Move(_, _)).WillByDefault(Invoke([this](const class Map&, Engine::Position destination)
        {
    ...
    MOCK_METHOD(void, Move, (const class Map& map, Engine::Position destination), (override));
