...
{
    MockStatted stats;
    EXPECT_CALL(stats, Get(Stat::hp)).WillRepeatedly(Return(Computation(5)));
    Counter counter("test", Stat::Id::hp);
...
{
    MockStatted stats;
    EXPECT_CALL(stats, Get(Stat::ap)).WillOnce(Return(Computation(3)));
    Counter counter("actions", Stat::Id::ap);
    EXPECT_EQ(counter.Available(0, stats), 3);
    EXPECT_CALL(stats, Get(Stat::ap)).WillOnce(Return(Computation(1)));
