...
TEST(Slice, CreateSlice)
{
    EXPECT_EQ(Slice(Material::stone, 1.0, 0.0)[0].material, Material::stone);
}
...
TEST(Slice, MixedSlice)
{
    ...
    EXPECT_EQ(slice[0].material, Material::stone);
    EXPECT_EQ(slice[1].material, Material::water);
    EXPECT_EQ(slice[2].material, Material::air);
}

TEST(Slice, StackMixesSlice)
{
    auto slice = Slice(Material::stone) + Slice(Material::air);
