...
TEST(Map, GroundHeight)
{
    ...
    EXPECT_FLOAT_EQ(high.GroundHeight({0, 1, 14.1}), 13.75f);
...
TEST(Map, HeightMap)
{
    ...
    map.SetHeightMap(Engine::Size(2,2,3), 
        { 0.5, 1.8,
          2.1, 0.9 });
    EXPECT_FLOAT_EQ(map.GroundHeight({0, 0, 0}), 0.5);
    EXPECT_FLOAT_EQ(map.GroundHeight({1, 0, 2}), 1.8);
    EXPECT_FLOAT_EQ(map.GroundHeight({0, 1, 3}), 2.1);
    EXPECT_FLOAT_EQ(map.GroundHeight({1, 1, 1}), 0.9);
}
