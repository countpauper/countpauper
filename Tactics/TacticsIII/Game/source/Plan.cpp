...
Plan Plan::Move(World& world, Avatar& actor, Position destination)
{
    Plan result;
    result.actions.emplace_back(std::move(std::make_unique<::Game::Move>(world, actor, destination)));
    return result;
}
...
{
    Plan result;
    result.actions.emplace_back(std::move(std::make_unique<::Game::Move>(world, actor, target.GetPosition(), actor.GetStats().Get(Stat::reach).Total())));
