...
Location::Location(Plane plane, unsigned pos, unsigned size) :
    plane(plane),
    position(pos),
    size(size)
{
...
Location::Location(Trajectory trajectory, unsigned position, unsigned range) :
    Location(ToPlane(trajectory), position, range)
{
}

// facing plane of target
Location::Location(const Direction& facing, const Direction& target, int position) :
    Location(Facing(target, facing), position)
{
}

// transform planes
Location::Location(Location attack, Location target) :
Location(Transform(attack.plane, target.plane), attack.position-target.position, attack.size)
{
