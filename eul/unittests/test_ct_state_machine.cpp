...
TEST(ct_state_machine, to_sub_state)
{
    ...
    // 3) [done] When executing the transition the variants should be constructed in sequence
    // 4) [done] The Parent states should support a constructor State(substates).
    ...
    // 7) [done] This is called to emplace a new variant *unless* the variant index is already the current variant (similar test to test_rt_state_machine)
    ...
    machine<Off, On> sm;
    sm.transition<Blinking>(warning);
