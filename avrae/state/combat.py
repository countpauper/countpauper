...
for c in C.combatants:
	...
	for field in fields:
		if field=='heart' and c.max_hp:
			...
			props.append(c.hp_str())
		elif field.startswith('effect'):
			for e in c.effects:
			...
			if field == 'distance':
				props.append(f'{int(distance)}ft')
			elif field=='melee':
				if distance <= ranges.reach:
					props.append(':crossed_swords:')
			elif field=='ranged':
				if distance<= ranges.range:
					props.append(':dart:')
				elif distance <= ranges.max_range:
					props.append(':bow_and_arrow:')
			elif field=='cantrip':
				if distance <= ranges.get('cantrip', -1):
					props.append(':magic_wand:')
			elif field=='spell':
				if distance <= ranges.get('spell', -1):
					props.append(':sparkles:')
		elif field=='team':
			if C.me and C.me.name==c.name:
				props.append('(You)')
			elif c.controller == ctx.author.id:
				props.append('(Yours)')
			elif c.controller==dm_id:
				props.append('(DM)')
			elif c.group:
