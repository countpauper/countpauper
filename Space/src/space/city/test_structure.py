import unittest
from specification import Building, Product
...
class Test_Container(unittest.TestCase):
    ...
    def test_store(self):
        building = Building(type=Storage, space=1)
        storage = building.create(location='None')
    ...
    def test_storage_amount(self):
        ...
        spaces = 3
        building = Building(type=Storage, space=1)
        storage = building.create(amount=spaces, location='None')
        product = Product(type=Food,storage=Storage)
